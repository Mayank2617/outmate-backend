...
async function processAndSave(rawData, categoryLabel) {
    ...
    for (const tweet of rawCandidates) {
        try {
            ...
            const metrics = {
                likes: tweet.counts?.favorite_count || legacy.favorite_count || 0,
                retweets: tweet.counts?.retweet_count || legacy.retweet_count || 0,
                replies: tweet.counts?.reply_count || legacy.reply_count || 0,
...
// 4. MAIN RUNNER
// ==========================================
async function runEngine(isStandalone = true) {
